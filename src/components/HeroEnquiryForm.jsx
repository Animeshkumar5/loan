    ...
    <div className="bg-white">
      <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-5">
        ...
        <div className="col-span-1 md:col-span-2 mt-2">
            <div className="flex items-start gap-3">
                ...
                <label htmlFor="terms" className="text-sm font-medium text-gray-700">
                    I agree to the <span className="text-gray-900 font-semibold">Terms and Conditions</span> and authorise the company to contact me via Email/SMS/WhatsApp. {" "}
                    <Link to="/privacy-policy" className="text-blue-600 hover:underline hover:text-blue-700 font-semibold">
                        Know More (Privacy Policy)
                    </Link>
                </label>
            </div>
        </div>
        ...
        <div className="col-span-1 md:col-span-2 pt-2">
          <button 
            type="submit"
            disabled={!isAgreed} // Disable button if not checked
            className={`w-full font-bold py-3.5 rounded-xl transition shadow-lg
                ${isAgreed 
                    ? 'bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 transform hover:scale-[1.02] active:scale-95 text-white cursor-pointer' 
      ...
      {status && (
        <div className={`mt-4 p-3 rounded-lg text-sm font-semibold text-center ${
          status.includes('successfully') || status.includes('Details saved') ? 'bg-green-50 text-green-700 border border-green-200' : 'bg-red-50 text-red-700 border border-red-200'
        }`}>
          {status}
        </div>
      )}
